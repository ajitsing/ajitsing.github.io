...
The event bus between write and read models is typically implemented with a message broker like Kafka (for high-volume streaming) or RabbitMQ (for routing flexibility). A [stream processing](/stream-processing/){:target="_blank" rel="noopener"} job is often what actually builds the read model. See [Kafka vs RabbitMQ vs SQS](/kafka-vs-rabbitmq-vs-sqs/) for help picking the right one. To make sure events actually reach the broker reliably, pair this with the [Transactional Outbox Pattern](/transactional-outbox-pattern/).
