...
**Redis Cluster complexity**: Scaling beyond a single machine requires Redis Cluster, which shards data using a fixed set of [hash slots](/distributed-systems/fixed-partitions/){:target="_blank" rel="noopener"} (0-16383 slots distributed across nodes). Multi-key commands like `MGET`, `MSET`, and transactions only work when all keys hash to the same slot. Workarounds like hash tags (`{user:123}:sessions`) help, but they add complexity and can create hot partitions if not used carefully.
