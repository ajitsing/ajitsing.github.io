...
**Cowork plus Claude Code together**: Use Cowork as the orchestrator and Claude Code as the implementer. Cowork handles project specs, research, and documentation in Word, Sheets, or Markdown. Claude Code handles the terminal: commits, builds, tests. Both can read a shared `CLAUDE.md` (or similar) that describes project goals, tech stack, and conventions. Put multi-step procedures in [Claude Code skills](/how-to-create-and-use-skills-in-claude-code/){:target="_blank" rel="noopener"} rather than stuffing them into that memory file, and enable those skills on claude.ai if a Cowork or cloud session needs them. You get one place for facts, playbooks that load on demand, and two agents that stay aligned. Ryan McDonald has written this up in detail; the pattern is "orchestrator for planning and docs, Code for code."
