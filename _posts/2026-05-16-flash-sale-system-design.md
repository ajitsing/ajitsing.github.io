...
2. **Charge.** The payment worker calls the gateway (Stripe, Adyen, Razorpay) with an idempotency key derived from the order id. If the charge succeeds, the order moves to `PAID`. If the charge fails, the order moves to `PAYMENT_FAILED` and a **compensating transaction** increments the Redis stock counter back. The deeper patterns behind that charge path, including the ledger and reconciliation, are covered in [Payment System Design](/payment-system-design/){:target="_blank" rel="noopener"}.
